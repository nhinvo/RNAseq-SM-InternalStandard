...
samples_df = pd.read_csv(snakemake.input['samples'], sep='\t', header=0, index_col='sample_name')
...
for organism in counts_df.index.unique(level='organism'):
    all_data['annotation_data'][organism] = counts_df.loc[organism]['sample_data'].to_dict(orient='index')
    all_data['count_data'][organism] = counts_df.loc[organism]['annotation_data'].to_dict(orient='index')
